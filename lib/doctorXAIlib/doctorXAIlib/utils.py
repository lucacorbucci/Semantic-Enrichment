...
def subset_ontology_from_data(icd9_tree, dataset_sequences):
    ...
    data_concepts = sorted(
        list(set([a for b in [c for d in dataset_sequences for c in d] for a in b])))
    data_nodes = []
...
def get_closest_datapoints(start_index, dist_matrix, k):
    candidates = sorted(
        enumerate(dist_matrix[start_index]), key=lambda x: x[1])[:k + 1]
...
def wup_visit(c2c, v1, v2):
    ...
    for icd9_1 in v1:
        for icd9_2 in v2:
            encoded_c2c[(encoder[icd9_1], encoder[icd9_2])
                        ] = 1 - c2c[(icd9_1, icd9_2)]
...
def flat_dp(patient, last_visit=True):
    ...
    if last_visit:
        ...
        for c in codes:
            # already rev
            seq = [1 if c in el else 0 for el in patient[:-1][::-1]]
...
def flat_cloud(kdp, last_visit=True, all_codes=None):
    ...
    if not all_codes:
        all_codes = sorted(
            list(set([a for b in [list(lf.keys()) for lf in local_flat] for a in b])))
...
def unflat_cloud(cloud, code_list, max_past, past_mean, past_std):
    ...
    for dp in cloud:  # iterate over synthetic datapoints
        ...
        raw_seq = [[code_list[c] for c in range(
            dp_mx.shape[0]) if dp_mx[c, v] == 1] for v in range(dp_mx.shape[1])]  #
...
def normal_perturbation_on_flat(flat_neigh, seed, size=1000):
    ...
    for code_col in range(num_codes):
        ...
        flat_syn_neigh[:, code_col] = np.random.normal(
            code_mean, code_std, size)
...
def ontological_perturbation_of_one_patient(patient, mask_prob, num_clones, G, seed):
    ...
    np.random.seed(seed)
    codes = list(set([a for b in patient for a in b]))
    exceptions = ['E849.0', 'E849.3', 'E849.4',
                  'E849.5', 'E849.6', 'E849.7', 'E849.8', 'E849.9']
    # subtree roots
    fringe = list(
        set([c if c in exceptions else c.split('.')[0] for c in codes]))
    # partition
    ...
    for k in range(num_clones):
        ...
        allowed_codes = [a for b in [c for i, c in enumerate(
            classes) if np.random.rand() > mask_prob] for a in b]
...
def old_dummify_patients(patient_seqs, dict_icd2int, n_unique_codes=4880):
    dummy_patients = np.array([old_dummify_patient(
        patient, dict_icd2int, n_unique_codes) for patient in patient_seqs])
...
def rule_extractor(DT, patient2bexplained, flat_patient, flat_patient_features_names, labels_names,
    ...
    decision_path_nodes = DT.tree_.decision_path(
        flat_patient.astype(np.float32))
    ...
    code_names = list()
    print("decision_path_nodes_indices: ")
    print(decision_path_nodes_indices)
    for node_id in decision_path_nodes_indices:
        ...
        if true_code_value > 0:
            ...
            if len(list_visits) == 1:
                ...
                node_string = node_string + \
                    f' was observed in visits {first_visits}'
