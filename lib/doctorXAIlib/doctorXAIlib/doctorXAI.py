...
class DoctorXAI():
    ...
    def find_closest_neighbors(self):
        ...
        if self.n_first_neighbors == 0:
            ...
            if self.p2p_distance_matrix:
                ...
                print(
                    'BE CAREFUL: using the precomputed patient2patient distance matrix')
                ...
                if self.c2c_distance_matrix:
                    ...
                    c2c_dict = compute_c2c(
                        self.icd9_tree, self.dataset_sequences, self.output_path, verbose=False)
                patients_dist = {}

                for i, patient in enumerate(self.dataset_sequences):
                    wup_patient_dist = wup_patient(
                        c2c_dict, self.patient_sequence, patient)
                ...
                real_neighs = [self.dataset_sequences[idx]
                               for idx in closest_neigh_indexes]
    ...
    def create_synthetic_neighborhood(self):
        ...
        if len(real_neighs) == 1:
            syn_neighs = ontological_perturbation_of_one_patient(
                real_neighs[0], seed=self.random_seed)
        else:
            ...
            syn_neighs = [
                syn_patient for syn_patient in synth_neigh_onto_seq if len(syn_patient) >= 2]
        ...
        syn_neighs.sort()
        syn_neighs = list(syn_neigh for syn_neigh,
                          _ in itertools.groupby(syn_neighs))
    ...
    def extract_rule(self, ICD9_description_dict=None, CCS_description_dict=None):
        ...
        flat_syn_neighs, features_names = flat_cloud(
            syn_neighs, last_visit=True)
        print("feature names neigh")
        print(features_names)

        flat_patient, features_names = flat_cloud(
            [self.patient_sequence], all_codes=features_names, last_visit=True)

        print("feature names flat_patient")
        print(features_names)

        print("patient sequence")
        print(self.patient_sequence)
        print(len(self.patient_sequence))

        print("flat patient")
        print(flat_patient)
        print(len(flat_patient))

        print("flat_syn_neighs")
        print(flat_syn_neighs)
        print(flat_syn_neighs[0])
        print(len(flat_syn_neighs[0]))

        # one-hot-encoding of the labels
        one_hot_syn_neigh_labels, labels_names = dummify_labels(
            syn_neigh_labels)
        ...
        fidelity_synth = f1_score(
            y_true=y_test, y_pred=DT_synth.predict(x_test), average='micro')
        # hit of the DT
        hit_synth = 1 - \
            distance.hamming(dummy_patient_label, DT_synth.predict(
                flat_patient.reshape(1, -1)))
        ...
        return decision_rule, istance_string, list_split_conditions, code_names, fidelity_synth, hit_synth, features_names, labels_names, DT_synth
    ...
    def visualize_explanation(self, save=False, figure_path='./explanation.png'):
        ...
        tree_prediction = self.decision_rule.split(
            '->')[1].replace(' ', '').split(',')
        print("bb_prediction:")
